...
T=Translator(model=model,tokenizer=model)
uuids=T.get_stories_uuids()
df=T.get_df()
start=0
try:
  with open('current_story_number.json', 'r') as f:
          data = json.load(f)
          start = data['start']
except Exception as e:
  print("no current files done")
print("start is ", start)
uuids=uuids[0:540]
for uuid in uuids:
  start_time = timeit.default_timer()
  text,uuid_story,author,title=T.get_story(uuid,df)
  #if len(text)>4200:#4200 #For GPU
  #  start+=1
  #  continue
  if os.path.isfile(f'./../Cuentos_english/{uuid_story}.json'):
    continue
  book = T.translate(text)
  T.save(book,uuid_story,title,author)
  start+=1
  data_num={"uuid_current":uuid_story,"start":start}
  with open('current_story_number.json', 'w') as f:
    json.dump(data_num, f)
  took = (timeit.default_timer() - start_time) * 1.0
